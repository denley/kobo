...
- 2a progress: the level format, manifest, import from ROM, and build are in
  ...
  (4.5 KiB), until 2b's Lunar Magic layout lifts that. Still to do in 2a: the staged
  build and its cache, BPS output, Asar for Kobo's own patches, and import from MWL.
